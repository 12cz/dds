...
t = linspace(0, 1/4, samp_num);
y = A * sin(2*pi*f*t +phi);
y(samp_num) = 1.0;
yint = int16(round(y*(2^15 -1)));
...
N_sample = round(f_s_1/f_o * 100);                    %绘制2个周期的波形
...
addr_shift = phase_bits - addr_width;        % = 36，被截断的低位数
...
for i = 1:N_sample
    ...
    switch quad
        ...
        case 1, gen_wave(i) =  tbl(samp_num - addr);
        case 2, gen_wave(i) = -tbl(addr + 1);
        case 3, gen_wave(i) = -tbl(samp_num - addr);
    end
    ...
    switch quad1
        ...
        case 1, gen_wave_dither(i) =  tbl(samp_num - addr1);
        case 2, gen_wave_dither(i) = -tbl(addr1 + 1);
        case 3, gen_wave_dither(i) = -tbl(samp_num - addr1);
    end
    ...
    % dither_val = (double(lfsr_data) - 128) * 2^addr_shift /512;
    % dither_byte = double(bitand(lfsr_data, uint32(0xFF)));%23bit低8bit
    dither_byte = double(bitshift(lfsr_data, -15));%23bit高8bit
